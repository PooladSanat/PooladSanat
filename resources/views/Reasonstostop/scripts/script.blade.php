...
<script type="text/javascript">
    $(function () {
        ...
        $('#createNewProduct').click(function () {
            ...
            $('#caption').text('افزودن دلیل توقف');
            $('#id').val('');
        });
        $('body').on('click', '.editProduct', function () {
            var id = $(this).data('id');
            $.get("{{ route('admin.Reasonstostop.update') }}" + '/' + id, function (data) {
                $('#ajaxModel').modal('show');
                $('#caption').text('ویرایش دلیل توقف');
                $('#id').val(data.id);
        ...
        $('#saveBtn').click(function (e) {
            ...
            $.ajax({
                ...
                url: "{{ route('admin.Reasonstostop.store') }}",
                type: "POST",
                dataType: 'json',
    ...
    $('body').on('click', '.deleteProduct', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{route('admin.Reasonstostop.delete')}}" + '/' + id,
                    data: {
